...
4. **Maps: hand-drawn renders canonical.** The May-2026 bare-name clean renders are the course maps (owner's explicit direction). Infographic variants are *referenced* in the Source Inventory as optional study posters, never embedded or copied. The former photo-slot maps (M01/M08/M10/M27) now ship as clean redrawn renders (redrawn 2026-06-12 from the course photo-source PNGs); the remaining framed-photo sources stay in the manifest redraw queue.
...
**DEFERRED (10 — source is a framed photo; image NOT embedded; teaching ships text-first; listed in the manifest's redraw queue):**
...
Producing/hosting actual video or audio (kits ship instead) · platform implementation (reference stack documented, not deployed) · advanced-tier teaching (stellating, intimacy offers, Fear Club, Distilling Destiny — horizons pointers only) · embedding infographic variants · redrawing the remaining deferred framed-photo maps · payment processing specifics beyond the terms/refund tree.
